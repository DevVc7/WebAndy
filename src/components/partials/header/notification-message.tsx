      ...
      <DropdownMenuTrigger asChild>
        <Button
          ...
          data-[state=open]:bg-green-100 
           hover:text-gray-700 text-green-500  rounded-full  "
        >
          <Bell className="h-8 w-8 " />
          <Badge className="w-4 h-4 p-0 text-xs  font-medium  items-center justify-center absolute left-[calc(100%-18px)] bottom-[calc(100%-16px)] ring-2 bg-green-500">
            2
          </Badge>
        </Button>
      </DropdownMenuTrigger>
          ...
          <ScrollArea className="h-full">
            {notifications.map((item, index) => (
              <DropdownMenuItem key={`inbox-${index}`} className="flex gap-9 py-2 px-4 cursor-pointer dark:hover:bg-background">
                ...
                <div
                  className={cn("text-xs font-medium text-gray-900 whitespace-nowrap",{ "text-green-600": !item.unreadmessage })}>
                  {item.date}
                </div>
                <div
